...
header(){
  ...
  echo -e "${C1}  ══════════════════════════════════════════════════════════════${NC}"
  echo -e "${C1}     ___   ____  _   _      ____   _    _   _  _____  _      _  ${NC}"
  echo -e "${C2}    / _ \\ / ___|| | | |    |  _ \\ / \\ | \\ | || ____|| |    | | ${NC}"
  echo -e "${C3}   | | | || |_   | |_| |    | |_) / _ \\ |  \\| ||  _|  | |    | | ${NC}"
  echo -e "${C4}   | |_| || |_| ||  _  |    |  __/ ___ \\| |\\  || |___ | |___ | |__${NC}"
  echo -e "${C5}    \\___/\\____||_| |_|    |_| /_/   \\_|_| \\_||_____||_____||____|${NC}"
  echo -e "${C5}  ══════════════════════════════════════════════════════════════${NC}"
  echo ""
...
menu_setting(){
  while true; do
    ...
    case "$CH" in
      1) menu_tema ;;
      2) read -rp "  Port SSH baru (contoh: 22, 2222): " P
      ...
      3) read -rp "  Port Dropbear baru (contoh: 109): " P
         if [[ ! "$P" =~ ^[0-9]+$ || "$P" -lt 1 || "$P" -gt 65535 ]]; then
