...
akun_tambah() {
    ...
    printf "  ${WHITE}%-12s${NC}: ${LGREEN}%s${NC}\n" "Port"     "5667"
    draw_line; echo
    tg_send "➕ *AKUN BARU*%0AUser: \`$U\` | Pass: \`$P\`%0AExp: $EF%0AIP: $IP_PUB%0AHost: $DOM:5667"
    read -rp "  ENTER..." _
}
...
main_menu() {
    while true; do
        ...
        # menu 9 tampilkan tema aktif
        _TM_NOW=$(tr '[:lower:]' '[:upper:]' < "$THEME_FILE" 2>/dev/null || echo 'RAINBOW')
        if [[ "$CUR_THEME" == "rainbow" ]]; then
            printf "  ${RB[2]}[9]${NC}  🎨  %-18s" "Ganti Tema Warna"
            printf " "; rb_text "[$_TM_NOW]"; echo -e "  ${DIM}┃${NC}"
        else
            printf "  ${P2}[9]${NC}  🎨  %-18s ${P2}[$_TM_NOW]${NC}  ${DIM}┃${NC}\n" "Ganti Tema Warna"
        fi
        echo -e "  ${LRED}[E]${NC}  🗑   Uninstall ZiVPN              ${DIM}┃${NC}"
        echo -e "  ${LRED}[0]${NC}  ✖   Keluar                       ${DIM}┃${NC}"
        echo; draw_line
